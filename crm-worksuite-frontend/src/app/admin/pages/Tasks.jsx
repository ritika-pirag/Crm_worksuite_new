    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      >
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {labels.map((label) => (
              <div key={label?.name || String(label)} className="flex items-center justify-between p-2 border border-gray-200 rounded-lg">
                <Badge className={getLabelColor(label)}>{label?.name || String(label)}</Badge>
                <button
                  onClick={() => {
                    const nameToRemove = label?.name || String(label)
                    setLabels(labels.filter(l => (l?.name || String(l)) !== nameToRemove))
                  }}
