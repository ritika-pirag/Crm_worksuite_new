...
const TopBar = ({ onMenuClick, isSidebarCollapsed, onToggleSidebar }) => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const response = await notificationsAPI.getAll({ company_id: companyId, limit: 10 })
    ...
    <>
      ...
      >
        <div className="px-4 lg:px-6 py-3 flex items-center justify-between w-full h-full gap-4">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 flex-shrink-0">
            ...
            <div className="flex items-center gap-3 cursor-pointer flex-shrink-0 group" onClick={() => navigate('/app/admin/dashboard')}>
              ...
              <span className="text-lg lg:text-xl font-bold text-primary-text whitespace-nowrap hidden sm:block">RISE</span>
            </div>
            ...
            <div className="relative ml-1 lg:ml-2" ref={dashboardMenuRef}>
              ...
              {showDashboardMenu && (
                <div
                  className="absolute top-full left-0 mt-2 w-64 bg-white rounded-xl shadow-elevated border border-gray-100 py-2 z-50 animate-fadeIn"
                >
                  <button
                    className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-3 transition-colors"
                    onClick={() => { navigate('/app/admin/dashboard'); setShowDashboardMenu(false); }}
                  >
                  ...
                  <button
                    className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-3 transition-colors"
                    onClick={() => { setShowDashboardMenu(false); alert('Custom Dashboard coming soon!') }}
                  >
                  ...
                  <button
                    ...
                    onClick={() => { setShowDashboardMenu(false); alert('Feature coming soon!') }}
                  >
          ...
          <div className="flex items-center gap-1.5 lg:gap-2 flex-shrink-0">
            ...
            {/* Add Button */}
            <button
              className="hidden sm:flex items-center justify-center p-2 lg:p-2.5 text-secondary-text hover:text-primary-text hover:bg-sidebar-hover rounded-xl transition-all duration-200 flex-shrink-0"
            >
              <IoAdd size={22} />
            </button>
            ...
            <div className="relative flex-shrink-0" ref={profileMenuRef}>
              ...
              {showProfileMenu && (
                <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-elevated border border-gray-200 overflow-hidden animate-fadeIn" style={{ position: 'fixed', top: '4.5rem', right: '1rem', zIndex: 10000 }}>
        ...
        <div className="md:hidden">
          <GlobalSearch isOpen={showSearch} onClose={() => setShowSearch(false)} mode="modal" />
        </div>
      </header>
